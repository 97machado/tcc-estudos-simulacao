...
", file = "Cenário 1 - Alcance/Modelo1Cen1Caso3.stan")

data_1_1_3 <- list(k = k, N = N, z = z, r = r, Mdist = Mdist)

initf2 <- function(chain_id = 1) {
  list(mu = chain_id, beta = chain_id, gamma = chain_id, tau = chain_id, w = rep(chain_id,100))
} 

Mod1Cen1Caso3 <- stan(file = "Cenário 1 - Alcance/Modelo1Cen1Caso3.stan", 
                      data = data_1_1_3, 
                      iter = 20000,
                      warmup = 5000, 
                      thin = 20,
...
print(Mod1Cen1Caso3)
res_1_1_3 <- rstan::extract(Mod1Cen1Caso3)
save.image("Cenário 1 - Alcance/Mod1Cen1Caso3.RData")

# Resultados ---------------------------------------------------------------------------------
estimativas <- data.frame(cbind(beta = c(beta, mean(res_1_1_3$beta), median(res_1_1_3$beta)),
                                tau = c(tau.sim, mean(res_1_1_3$tau), median(res_1_1_3$tau)),
                                gamma = c(gamma.sim, mean(res_1_1_3$gamma), 
                                          median(res_1_1_3$gamma)),
                                mu = c(mu, mean(res_1_1_3$mu), median(res_1_1_3$mu))), 
...
DFplots <- data.frame(iter = 1:1500,
                      ...
                      mu = res_1_1_3$mu,
                      tau = res_1_1_3$tau,
                      gamma = res_1_1_3$gamma)

# Beta
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_hist", pars = c("beta"))
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_trace", pars = c("beta"))
...
# Mu
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_hist", pars = c("mu"))
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_trace", pars = c("mu"))
...
# Tau
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_hist", pars = c("tau"))
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_trace", pars = c("tau"))
...
# Gamma
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_hist", pars = c("gamma"))
rstan::plot(Mod1Cen1Caso3, plotfun = "stan_trace", pars = c("gamma"))
...
# Cadeia Forte
set.seed(3); sample(1:100, size = 4) # w[5], w[12], w[36] e w[58]
...
# w[5]
ggplot() +
  geom_path(aes(x = iter, y = w5), colour = "blue", data = DFplots_w) +
  geom_hline(yintercept = dados.sim$w[5], linetype = "dashed", color = "red", size = 2) +
  xlab("Iterações") +
  ylab(bquote(paste('Processo Gaussiano w'['5']*''))) +
  theme_bw() +
  theme(axis.title.x = element_text(size = rel(2)),
        legend.text = element_text(size = rel(2)),
        axis.title.y = element_text(size = rel(2)),
...
# w[12]
ggplot() +
  geom_path(aes(x = iter, y = w12), colour = "blue", data = DFplots_w) +
  geom_hline(yintercept = dados.sim$w[12], linetype = "dashed", color = "red", size = 2) +
  xlab("Iterações") +
  ylab(bquote(paste('Processo Gaussiano w'['12']*''))) +
  theme_bw() +
  theme(axis.title.x = element_text(size = rel(2)),
        legend.text = element_text(size = rel(2)),
        axis.title.y = element_text(size = rel(2)),
...
# w[36]
ggplot() +
  geom_path(aes(x = iter, y = w36), colour = "blue", data = DFplots_w) +
  geom_hline(yintercept = dados.sim$w[36], linetype = "dashed", color = "red", size = 2) +
  xlab("Iterações") +
  ylab(bquote(paste('Processo Gaussiano w'['36']*''))) +
  theme_bw() +
  theme(axis.title.x = element_text(size = rel(2)),
        legend.text = element_text(size = rel(2)),
        axis.title.y = element_text(size = rel(2)),
...
# w[58]
ggplot() +
  geom_path(aes(x = iter, y = w58), colour = "blue", data = DFplots_w) +
  geom_hline(yintercept = dados.sim$w[58], linetype = "dashed", color = "red", size = 2) +
  xlab("Iterações") +
  ylab(bquote(paste('Processo Gaussiano w'['58']*''))) +
  theme_bw() +
  theme(axis.title.x = element_text(size = rel(2)),
        legend.text = element_text(size = rel(2)),
        axis.title.y = element_text(size = rel(2)),
...
# w
g1w_1_1_3 <- data.frame(x = seq(1,100,1),
                        media = apply(res_1_1_3$w, 2, mean)[seq(1,100,1)],
                        mediana = apply(res_1_1_3$w, 2, median)[seq(1,100,1)],
                        real = dados.sim$w[seq(1,100,1)],
                        inf = apply(res_1_1_3$w, 2, 
                                    function(x) quantile(x, 0.025))[seq(1,100,1)],
                        sup = apply(res_1_1_3$w, 2, 
                                    function(x) quantile(x, 0.975))[seq(1,100,1)])

Val.g1w_1_1_3 <- g1w_1_1_3 %>%
...
ggplot() +
  geom_point(aes(x = x, y = valor, group = ind, shape = ind, color = ind), 
             size = 2, data = Val.g1w_1_1_3) +
  geom_errorbar(aes(x = x, ymax = sup, ymin = inf), width = 0, data = g1w_1_1_3) +
  xlab("Sub-regiões") +
...
ggplot() +
  geom_point(aes(x = real, y = media), size = 2, data = g1w_1_1_3) +
...
g1w_1_1_3$cx <- dados.sim$x
g1w_1_1_3$cy <- dados.sim$y
g1w_1_1_3$contido <- ifelse((g1w_1_1_3$real >= g1w_1_1_3$inf) & 
                              (g1w_1_1_3$real <= g1w_1_1_3$sup), 1, 0)
table(g1w_1_1_3$contido)
ForaICForte <- g1w_1_1_3 %>% filter(contido == 0) %>% select(cx, cy) %>% 
...
plot(qS, add = T, col = "", cex = 1.5, lty = 2, lwd = 2)
plot(S, add = T, cex = 1.5, lty = 2, pch = 20, lwd = 1)
plot(ForaICForte, add = T, pch = "x", col = "red", cex = 1.5)

#load("Cenário 1 - Alcance/Mod1Cen1Caso3.RData")
save.image("Cenário 1 - Alcance/Mod1Cen1Caso3.RData")
