...
x <- rep(seq(1, 9, by = 2), 5)
y <- rep(1, 5); for (i in 1:4) y <- c(y, rep(1 + (i*2), 5))
...
w <- mvrnorm(n = 1, mu = mu * rep(1, 25), Sigma = Sigma.sim)
dados.sim$w <- w

# 9. Gerando a covariável z
z <- mvrnorm(n = 1, mu = rep(0, 25), Sigma = diag(25))
dados.sim$z <- z
...
r <- rep(1, 25) # Considerando r(s) = 1, para todo s
dados.sim$r <- r
...
", file = "Cenário 2 - Discretização/Modelo1Cen2Caso3.stan")

data_1_2_3 <- list(k = k, N = N, z = z, r = r, Mdist = Mdist)

initf2 <- function(chain_id = 1) {
  list(mu = chain_id, beta = chain_id, gamma = chain_id, tau = chain_id, w = rep(chain_id,25))
} 

Mod1Cen2Caso3 <- stan(file = "Cenário 2 - Discretização/Modelo1Cen2Caso3.stan", 
                      data = data_1_2_3, 
                      iter = 20000,
                      warmup = 5000, 
                      thin = 20,
...
print(Mod1Cen2Caso3)
res_1_2_3 <- rstan::extract(Mod1Cen2Caso3)
#save.image("Cenário 2 - Discretização/Mod1Cen2Caso3.RData")

# Resultados --------------------
estimativas <- data.frame(cbind(beta = c(beta, mean(res_1_2_3$beta), median(res_1_2_3$beta)),
                                tau = c(tau.sim, mean(res_1_2_3$tau), median(res_1_2_3$tau)),
                                gamma = c(gamma.sim, mean(res_1_2_3$gamma), median(res_1_2_3$gamma)),
                                mu = c(mu, mean(res_1_2_3$mu), median(res_1_2_3$mu))), row.names = c("valor real","média","mediana"))
...
# Beta
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_hist", pars = c("beta"))
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_trace", pars = c("beta"))
...
# Mu
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_hist", pars = c("mu"))
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_trace", pars = c("mu"))
...
# Tau
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_hist", pars = c("tau"))
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_trace", pars = c("tau"))
...
# Gamma
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_hist", pars = c("gamma"))
rstan::plot(Mod1Cen2Caso3, plotfun = "stan_trace", pars = c("gamma"))
...
# w
g1w_1_2_3 <- data.frame(x = seq(1,25,1),
                        media = apply(res_1_2_3$w, 2, mean)[seq(1,25,1)],
                        mediana = apply(res_1_2_3$w, 2, median)[seq(1,25,1)],
                        real = dados.sim$w[seq(1,25,1)],
                        inf = apply(res_1_2_3$w, 2, function(x) quantile(x, 0.025))[seq(1,25,1)],
                        sup = apply(res_1_2_3$w, 2, function(x) quantile(x, 0.975))[seq(1,25,1)])


Val.g1w_1_2_3 <- g1w_1_2_3 %>% pivot_longer(cols = 2:6, names_to = "ind", values_to = "valor") %>% filter(ind %in% c("media","real"))

ggplot() +
  geom_point(aes(x = x, y = valor, group = ind, shape = ind, color = ind), size = 2, data = Val.g1w_1_2_3) +
  geom_errorbar(aes(x = x, ymax = sup, ymin = inf), data = g1w_1_2_3) +
  xlab("Sub-regiões") +
...
g2w_1_2_3 <- data.frame(valor.real = dados.sim$w,
                        valor.media = apply(res_1_2_3$w, 2, mean))
ggplot() +
  geom_point(aes(x = valor.real, y = valor.media), size = 2, data = g2w_1_2_3) +
...
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_dens")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_trace")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_hist")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_diag")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_scat", pars = c("gamma","tau"))
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_rhat")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_ess")
# rstan::plot(Mod1Cen2Caso3, plotfun = "stan_mcse")
...
plot(S, pch = 20, main="")
qS <- quadratcount(S, 5, 5)
plot(qS, add = T, col = "", cex = 1.5, lty = 2)
dados.sim$N

#load("Cenário 2 - Discretização/Mod1Cen2Caso3.RData")
#save.image("Cenário 2 - Discretização/Mod1Cen2Caso3.RData")

#Salvando objetos para comparação entre simulações
dados.sim_1_2_3 <- dados.sim
gamma.sim_1_2_3 <- gamma.sim
rm(list = ls()[!(ls() %in% c("res_1_2_3","dados.sim_1_2_3","gamma.sim_1_2_3"))])
