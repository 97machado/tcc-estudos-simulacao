...
", file = "Cenário 2 - Discretização/Modelo1Cen2Caso1.stan")

data_1_2_1 <- list(k = k, N = N, z = z, r = r, Mdist = Mdist)

initf2 <- function(chain_id = 1) {
  list(mu = chain_id, beta = chain_id, gamma = chain_id, tau = chain_id, w = rep(chain_id,100))
} 

Mod1Cen2Caso1 <- stan(file = "Cenário 2 - Discretização/Modelo1Cen2Caso1.stan", 
                      data = data_1_2_1, 
                      iter = 20000,
                      warmup = 5000, 
                      thin = 20,
...
print(Mod1Cen2Caso1)
res_1_2_1 <- rstan::extract(Mod1Cen2Caso1)
#save.image("Cenário 2 - Discretização/Mod1Cen2Caso1.RData")

# Resultados --------------------
estimativas <- data.frame(cbind(beta = c(beta, mean(res_1_2_1$beta), median(res_1_2_1$beta)),
                                tau = c(tau.sim, mean(res_1_2_1$tau), median(res_1_2_1$tau)),
                                gamma = c(gamma.sim, mean(res_1_2_1$gamma), median(res_1_2_1$gamma)),
                                mu = c(mu, mean(res_1_2_1$mu), median(res_1_2_1$mu))), row.names = c("valor real","média","mediana"))
...
DFplots <- data.frame(iter = 1:1500,
                      ...
                      mu = res_1_2_1$mu,
                      tau = res_1_2_1$tau,
                      gamma = res_1_2_1$gamma)

# Beta
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_hist", pars = c("beta"))
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_trace", pars = c("beta"))
...
# Mu
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_hist", pars = c("mu"))
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_trace", pars = c("mu"))
...
# Tau
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_hist", pars = c("tau"))
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_trace", pars = c("tau"))
...
# Gamma
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_hist", pars = c("gamma"))
rstan::plot(Mod1Cen2Caso1, plotfun = "stan_trace", pars = c("gamma"))
...
# w
g1w_1_2_1 <- data.frame(x = seq(1,100,3),
                        media = apply(res_1_2_1$w, 2, mean)[seq(1,100,3)],
                        mediana = apply(res_1_2_1$w, 2, median)[seq(1,100,3)],
                        real = dados.sim$w[seq(1,100,3)],
                        inf = apply(res_1_2_1$w, 2, function(x) quantile(x, 0.025))[seq(1,100,3)],
                        sup = apply(res_1_2_1$w, 2, function(x) quantile(x, 0.975))[seq(1,100,3)])


Val.g1w_1_2_1 <- g1w_1_2_1 %>% pivot_longer(cols = 2:6, names_to = "ind", values_to = "valor") %>% filter(ind %in% c("media","real"))

ggplot() +
  geom_point(aes(x = x, y = valor, group = ind, shape = ind, color = ind), size = 2, data = Val.g1w_1_2_1) +
  geom_errorbar(aes(x = x, ymax = sup, ymin = inf), data = g1w_1_2_1) +
  xlab("Sub-regiões") +
...
g2w_1_2_1 <- data.frame(valor.real = dados.sim$w,
                        valor.media = apply(res_1_2_1$w, 2, mean))
ggplot() +
  geom_point(aes(x = valor.real, y = valor.media), size = 2, data = g2w_1_2_1) +
...
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_dens")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_trace")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_hist")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_diag")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_scat", pars = c("gamma","tau"))
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_rhat")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_ess")
# rstan::plot(Mod1Cen2Caso1, plotfun = "stan_mcse")
...
#load("Cenário 2 - Discretização/Mod1Cen2Caso1.RData")
#save.image("Cenário 2 - Discretização/Mod1Cen2Caso1.RData")

#Salvando objetos para comparação entre simulações
dados.sim_1_2_1 <- dados.sim
gamma.sim_1_2_1 <- gamma.sim
res_1_2_1 <- res_1_1_2
Mod1Cen2Caso1 <- Mod1Cen1Caso2
rm(list = ls()[!(ls() %in% c("res_1_2_1","dados.sim_1_2_1","gamma.sim_1_2_1"))])
